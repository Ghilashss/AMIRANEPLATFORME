...
exports.createLivraison = async (req, res) => {
    try {
        ...
        // ===================================================
        // 💰 VIREMENTS AUTOMATIQUES APRÈS VALIDATION
        // ===================================================
        console.log('💰 Début des virements automatiques...');
        
        try {
            // 1️⃣ VIREMENT PRIX COLIS → COMMERCANT
            if (colis.expediteur && colis.expediteur.id && colis.montant > 0) {
                console.log(`💵 Virement ${colis.montant} DA vers commerçant ${colis.expediteur.id}`);
                
                // Trouver portefeuille agent
                const portefeuilleAgent = await Portefeuille.findOne({
                    proprietaireId: req.user.agence,
                    typeProprietaire: 'Agence'
                });
                
                // Trouver portefeuille commercant
                let portefeuilleCommercant = await Portefeuille.findOne({
                    proprietaireId: colis.expediteur.id,
                    typeProprietaire: 'User'
                });
                
                // Créer portefeuille commercant si inexistant
                if (!portefeuilleCommercant) {
                    const commercant = await User.findById(colis.expediteur.id);
                ...
                if (portefeuilleAgent && portefeuilleAgent.solde >= colis.montant) {
                    // Créer transaction virement vers commercant
                    const virementCommercant = new OperationFinanciere({
                        type: 'virement',
                        ...
                        expediteur: {
                            id: req.user.agence,
                            type: 'Agence',
                            nom: req.user.agenceNom || 'Agence'
                        },
                        destinataire: {
                            id: colis.expediteur.id,
                            type: 'User',
                            nom: colis.expediteur.nom || 'Commerçant'
                        },
                        description: `Prix colis ${colis.tracking} (livré)`,
                        statut: 'validee',
                        colisId: colis._id,
                        referenceTransaction: `PRIX-${colis.tracking}-${Date.now()}`
                    });
                    
                    await virementCommercant.save();
                    
                    // Mettre à jour les soldes
                    portefeuilleAgent.solde -= colis.montant;
                    portefeuilleCommercant.solde += colis.montant;
                    await portefeuilleAgent.save();
                    await portefeuilleCommercant.save();
                    
                    console.log(`✅ Virement ${colis.montant} DA → Commerçant effectué`);
                } else {
                    console.log(`⚠️ Solde agent insuffisant: ${portefeuilleAgent?.solde || 0} DA < ${colis.montant} DA`);
                }
            }
            
            // 2️⃣ VIREMENT FRAIS LIVRAISON → ADMIN
            if (colis.fraisLivraison > 0) {
                console.log(`💵 Virement frais ${colis.fraisLivraison} DA vers Admin`);
                ...
                // Trouver portefeuille admin (typeProprietaire: 'User', role: 'admin')
                const adminUser = await User.findOne({ role: 'admin' });
                let portefeuilleAdmin = null;
                
                if (adminUser) {
                    portefeuilleAdmin = await Portefeuille.findOne({
                        proprietaireId: adminUser._id,
                        typeProprietaire: 'User'
                    });
                    
                    // Créer portefeuille admin si inexistant
                ...
                if (portefeuilleAgent && portefeuilleAdmin && portefeuilleAgent.solde >= colis.fraisLivraison) {
                    // Créer transaction virement vers admin
                    const virementAdmin = new OperationFinanciere({
                        type: 'virement',
                        montant: colis.fraisLivraison,
                        portefeuilleSource: portefeuilleAgent._id,
                        portefeuilleDestination: portefeuilleAdmin._id,
                        expediteur: {
                            id: req.user.agence,
                            type: 'Agence',
                            nom: req.user.agenceNom || 'Agence'
                        ...
                        description: `Frais livraison ${colis.tracking}`,
                        statut: 'validee',
                        colisId: colis._id,
                        referenceTransaction: `FRAIS-${colis.tracking}-${Date.now()}`
                    });
                    
                    await virementAdmin.save();
                    
                    // Mettre à jour les soldes
                    portefeuilleAgent.solde -= colis.fraisLivraison;
                    portefeuilleAdmin.solde += colis.fraisLivraison;
                    await portefeuilleAgent.save();
                    await portefeuilleAdmin.save();
                    
                    console.log(`✅ Virement ${colis.fraisLivraison} DA → Admin effectué`);
                } else {
                    console.log(`⚠️ Solde agent insuffisant pour frais: ${portefeuilleAgent?.solde || 0} DA < ${colis.fraisLivraison} DA`);
                }
            }
            
            console.log('✅ Virements automatiques terminés');
            
        } catch (virementError) {
            console.error('❌ Erreur lors des virements automatiques:', virementError);
            // Ne pas bloquer la livraison si les virements échouent
        }
